...
class CompanionState(
    context: Context,
    private val errors: ErrorReporter = agentsErrorReporter(context.applicationContext),
) {
    private val preferences = context.getSharedPreferences(PREFERENCES, Context.MODE_PRIVATE)
    fun saveAgents(agents: List<AgentSummary>) { commit("saving the agent list") { putString(KEY_AGENTS, RouterProtocol.agentsToJson(agents)); putLong(KEY_AGENTS_UPDATED_AT, System.currentTimeMillis()) } }
    fun loadAgents(): List<AgentSummary> = parse("parsing the saved agent list", emptyList()) {
        preferences.getString(KEY_AGENTS, null)?.let(RouterProtocol::parseAgents) ?: emptyList()
    }
    fun agentsUpdatedAt() = preferences.getLong(KEY_AGENTS_UPDATED_AT, 0L)
    fun saveDoctor(status: DoctorStatus) { commit("saving router doctor status") { putBoolean(KEY_DOCTOR_OK, status.ok); putString(KEY_DOCTOR_SUMMARY, status.summary); putLong(KEY_DOCTOR_UPDATED_AT, System.currentTimeMillis()) } }
    fun loadDoctor(): DoctorStatus? = parse("reading saved router doctor status", null) {
        preferences.getString(KEY_DOCTOR_SUMMARY, null)?.let { DoctorStatus(preferences.getBoolean(KEY_DOCTOR_OK, false), it) }
    }
    fun doctorUpdatedAt() = preferences.getLong(KEY_DOCTOR_UPDATED_AT, 0L)
    fun savePebbleSession(session: PebbleSession) { commit("saving the open watch session") { putString(KEY_PEBBLE_UUID, session.appUuid.toString()); putString(KEY_PEBBLE_WATCH, session.watchId); putLong(KEY_PEBBLE_OPENED_AT, System.currentTimeMillis()) } }
    fun loadPebbleSession(): PebbleSession? = parse("parsing the saved watch session", null) {
        val uuid = preferences.getString(KEY_PEBBLE_UUID, null) ?: return@parse null
        val watch = preferences.getString(KEY_PEBBLE_WATCH, null) ?: return@parse null
        PebbleSession(java.util.UUID.fromString(uuid), watch)
    }
    fun clearPebbleSession(session: PebbleSession) { if (loadPebbleSession() == session) commit("clearing the open watch session") { remove(KEY_PEBBLE_UUID); remove(KEY_PEBBLE_WATCH) } }
    fun pebbleOpenedAt() = preferences.getLong(KEY_PEBBLE_OPENED_AT, 0L)
    fun saveBridgeError(message: String) { commit("saving companion error status") { putString(KEY_BRIDGE_ERROR, message) } }
    fun loadBridgeError() = parse("reading saved companion error status", "") {
        preferences.getString(KEY_BRIDGE_ERROR, "").orEmpty()
    }

    fun sensitiveErrorValues(): List<String> = synchronized(HISTORY_LOCK) {
        buildList {
            loadTurn()?.text?.takeIf(String::isNotBlank)?.let(::add)
            loadAllHistory().map(CachedMessage::text).filter(String::isNotBlank).forEach(::add)
        }
    }
    ...
    fun appendHistory(message: CachedMessage) = synchronized(HISTORY_LOCK) {
        ...
        commit("saving agent message history") { putString(KEY_HISTORY, historyJson(retained).toString()) }
    }
    ...
    private fun saveTurn(turn: StoredTurn): Boolean = commit("saving an agent turn") { putString(KEY_TURN, turnJson(turn).toString()) }
    fun loadTurn(): StoredTurn? = parse("parsing the saved agent turn", null) {
        preferences.getString(KEY_TURN, null)?.let { parseTurn(JSONObject(it)) }
    }
    ...
    fun saveResult(result: StoredResult) { commit("saving a Termux result") {
        val json = JSONObject().put("requestId", result.requestId).put("kind", result.kind).put("mode", result.mode?.name).put("stdout", result.stdout.take(300000)).put("stderr", result.stderr.take(8192)).put("exitCode", result.exitCode).put("errorCode", result.errorCode).put("errorMessage", result.errorMessage.take(2048))
        putString("result_${result.kind}", json.toString())
    } }
    fun loadResult(kind: String = TermuxCommandRunner.KIND_SEND): StoredResult? = parse("parsing a saved Termux result", null) {
        preferences.getString("result_$kind", null)?.let { raw -> val j=JSONObject(raw); StoredResult(j.getString("requestId"),j.getString("kind"),j.optString("mode").takeIf(String::isNotEmpty)?.let(ExecutionMode::valueOf),j.optString("stdout"),j.optString("stderr"),j.optInt("exitCode",-1),j.optInt("errorCode"),j.optString("errorMessage")) }
    }
    ...
    private fun loadAllHistory(): List<CachedMessage> = parse("parsing saved agent message history", emptyList()) {
        preferences.getString(KEY_HISTORY, null)?.let { raw ->
    ...
    private fun commit(whileDoing: String, update: SharedPreferences.Editor.() -> Unit): Boolean = try {
        check(preferences.edit().apply(update).commit()) { "SharedPreferences commit returned false." }; true
    } catch (error: Throwable) { errors.report(error, whileDoing); false }
    private fun <T> parse(whileDoing: String, fallback: T, read: () -> T): T = try { read() }
        catch (error: Throwable) { errors.report(error, whileDoing); fallback }
