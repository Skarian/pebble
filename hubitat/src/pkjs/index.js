...
function refresh(requestId) {
  ...
  maker.devices(settings, function (error, response) {
    ...
    var devices = Model.normalizeDevices(response, settings.deviceIds || []);
...
function control(payload) {
  ...
  if ((settings.deviceIds || []).map(String).indexOf(deviceId) === -1) {
    sendStatus(STATUS.COMMAND_FAILURE, requestId, 'Device is not selected', CMD_RESULT);
    return;
  }
...
function settingsPage(settings) {
  var base = escapeHtml(settings.baseUrl || '');
  var hasToken = Boolean(settings.token);
  var selected = escapeHtml((settings.deviceIds || []).join(','));
  return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    ...
    '<body><main><h1>Hubitat</h1><p>Connect Maker API and choose up to six devices.</p>' +
    '<div class="privacy">Your access token stays in this app on your phone. It is never sent to the watch.</div>' +
    '<form id="form"><label for="url">Maker API URL</label><input id="url" required placeholder="http://HUB_IP/apps/api/APP_ID" value="' + base + '">' +
    '<p class="note">Copy the local or cloud URL from Maker API. Leave off <code>/devices</code> and <code>?access_token=</code>.</p>' +
...
Pebble.addEventListener('webviewclosed', function (event) {
  ...
  try {
    ...
    values.deviceIds = (values.deviceIds || []).slice(0, Model.MAX_DEVICES);
    if (!values.deviceIds.length || values.deviceIds.some(function (id) { return !/^\d+$/.test(id); })) {
      throw new Error('Enter one to six numeric device IDs');
    }
