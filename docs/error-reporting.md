...
Agents, CPAP, and Air Quality can send source errors to Pebble Diagnostics.
...
- Agents or Air Quality: open the Android companion and choose **Configure
  error reporting**.
- CPAP: open the watchapp's configuration page and use **Error reporting**.
...
Android keeps at most 128 queued records / 256 KiB per companion. CPAP keeps
at most 50 / 64 KiB in its private PebbleKit JS storage. Watch staging is
1.5 KiB for Agents and 2 KiB for CPAP and Air Quality; it is drained to the
phone whenever the watchapp and its phone runtime are connected. Queue
